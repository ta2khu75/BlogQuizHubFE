...
type Props<T extends string | number> = {
  ...
  isOptionEqual: (a: Option<T>) => boolean;
...
export const Combobox = <T extends string | number>({
  ...
  useEffect(() => {
    if (isDebounced && onInputChange) {
      onInputChange(debouncedInput);
    }
  }, [isDebounced, debouncedInput, onInputChange]);
              ...
              {filtered.map((option, index) => (
                ...
                >
                  ...
                  <Check
                    className={cn(
                      "ml-auto",
                      isOptionEqual(option) ? "opacity-100" : "opacity-0"
                    )}
                  />
                </CommandItem>
              ))}
